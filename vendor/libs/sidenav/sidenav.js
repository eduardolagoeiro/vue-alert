...
class SideNav {
	...
	_findUnopenedParent(item, closeChildren) {
		...
		for (let i = 0, l = tree.length; i < l; i++) {
			...
			if (this._accordion) {
				const openedItems = this._findChild(tree[i].parentNode, ['sidenav-item', 'open']);
				...
				for (let j = 0, k = openedItems.length; j < k; j++) {
					...
					if (closeChildren) {
						const openedChildren = openedItems[j].querySelectorAll('.sidenav-item.open');
	...
	_bindAnimationEndEvent(el, handler) {
		...
		el._sideNavAnimationEndEventTimeout = setTimeout(() => {
			cb({ target: el });
		}, duration + 50);
	}
